...
impl SsTableBuilder {
    ...
    pub fn build_from_iterator(&mut self, mut iter: Box<dyn KvIterator>) -> Result<(), WriterError> {
...
mod tests {
    ...
    fn test_builder_invalid_path_returns_error() {
        ...
        assert!(result.is_err(), "Builder should return an IO error for invalid paths");
    ...
    fn test_builder_single_block() {
        ...
        assert_eq!(first_key_entry.user_key, "apple", "Index should track the first key of the block");
        ...
        assert_eq!(data_block_offset, 0, "The first data block should start at byte 0");

        // 2. Verify Data Block
        let data_block_bytes = data[0..index_offset].to_vec();
        let data_block = Block::decode(data_block_bytes);
        
        assert_eq!(data_block.get_num_offsets().unwrap(), 3, "Data block should contain 3 entries");
    ...
    fn test_builder_multiple_blocks() {
        ...
        assert!(num_data_blocks > 1, "Builder should have created multiple data blocks");
        ...
        for i in 0..num_data_blocks {
            ...
            let block_end = if i + 1 < num_data_blocks {
                ...
                let (_, next_pointer_record) = index_block.decode_entry(next_idx_entry_offset).unwrap();
            ...
            let first_key_offset = data_block.get_offset(0, data_block.get_num_offsets().unwrap()).unwrap();
            let (actual_first_key, _) = data_block.decode_entry(first_key_offset).unwrap();
            
            assert_eq!(
                idx_key.user_key, actual_first_key.user_key, 
                "Index block key does not match actual first key of data block {}", i
            );
        }
    }
}